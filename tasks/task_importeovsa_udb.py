...
def uv_hex_rm(uv=None):
    import re
    # uvitems = re.sub(r'^r\s+', ' ', uv['vartable'])
    # uvitems = re.sub(r'\n[a-z]*\s*', ' ', uvitems)
...
def creatms(idbfile, outpath, timebin=None, width=None):
    ...
    sm.settimes(integrationtime='{:.3f}s'.format(delta_time),
                usehourangle=False,
...
def importeovsa(idbfiles, timebin=None, width=None, visprefix=None, nocreatms=True, doconcat=False, modelms=''):
    ...
    for filename in filelist:
        ...
        delta_time = 60.0
        end_time = uv['ntimes'] * delta_time
        time_steps = int(round((end_time - start_time) / delta_time))
        time_concat.append(time_steps)
        time0 = time.time()
        ...
        msname = visprefix + ''.join(msname) + '-{:d}m.ms'.format(int(round(delta_time*time_steps/60.0)))
        ...
        timearr = np.arange((time_steps), dtype=np.float)*delta_time
